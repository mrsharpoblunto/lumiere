  ...
  return {
    ...
    run: (matrix, audio, _dt, t) => {
      ...
      if (t >= sunRiseStart && t <= sunRisePeak) {
        ...
        paletteLerp = (t - sunRiseStart) / (sunRisePeak - sunRiseStart);
        audio.queue("bonsai.mp3");
      } else if (t > sunRisePeak && t < sunRiseEnd) {
        // sunrise -> day
        palette1 = sunxPalette;
        palette2 = dayPalette;
        paletteLerp = (t - sunRisePeak) / (sunRiseEnd - sunRisePeak);
        audio.queue("bonsai.mp3");
        ...
        palette1 = dayPalette;
        palette2 = sunxPalette;
        paletteLerp = (t - sunSetStart) / (sunSetPeak - sunSetStart);
        audio.queue("bonsai-night.mp3");
      } else if (t > sunSetPeak && t < sunSetEnd) {
        // sunset -> night
        palette1 = sunxPalette;
        palette2 = nightPalette;
        paletteLerp = (t - sunSetPeak) / (sunSetEnd - sunSetPeak);
        audio.queue("bonsai-night.mp3");
      } else {
        // night
        ...
        if (t < sunSetEnd) {
          paletteLerp = (t + (86400 * 1000 - sunSetEnd)) / interval;
        } else {
          paletteLerp = (t - sunSetEnd) / interval;
        }
        audio.queue("bonsai-night.mp3");
      }
